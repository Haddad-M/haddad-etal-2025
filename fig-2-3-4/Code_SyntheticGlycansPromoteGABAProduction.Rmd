...
for (i in sp){
  ...
  print(cor.test(sub[,1], sub[,2])$p.value)
...
# View correlation dataframe by absolute cor to check for GABA consumers
print(correlation[order(-abs(correlation$cor)),][1:10,]) # No high correlation (>0.5) GABA consumers. Note: Evtepia abundance is >70% zeros for SG-Glc conditions
...
# Subset for likely SG-Glc GABA-modulating species. 

```{r}
# Subset for sg-glc only
# The purpose of this subset is for the modeling step in the rmd chunk below this one
...
# Subset species_sg-glc_subset to include columns containing "gaba", "bacteroides", "bifidobacterium", "lactobacillus", "streptococcus", "evtepia"
# These are likely to have relationship with GABA
species_sgglc_subset = species_sgglc_subset[,c(grep("gaba", colnames(species_sgglc_subset)), 
                                               grep("Bifidobacterium", colnames(species_sgglc_subset)), 
                                               grep("Lactobacillus", colnames(species_sgglc_subset)), 
                                               grep("Streptococcus", colnames(species_sgglc_subset)), 
                                               grep("Bacteroides", colnames(species_sgglc_subset)),
                                               grep("Evtepia", colnames(species_sgglc_subset)))]

# Remove columns >70% zeros (13 or more zeros out of 18 conditions)
columns_to_remove = sapply(species_sgglc_subset, count_zeros) >= 13
species_sgglc_cleaned = species_sgglc_subset[, !columns_to_remove]
#print(columns_to_remove) # Uncomment and run this line to see species removed

# Calculate correlation coefficient between gaba and each species based on the subsetted conditions
# Since this is using condition subset, correlation coefficient values differ slightly from previously calculated
correlations = data.frame(sapply(species_sgglc_cleaned, function(x) cor(species_sgglc_cleaned$gaba, x)))
colnames(correlations)[1] = "corr"
correlations = correlations[order(-correlations$corr), , drop = FALSE]
#print(correlations[!row.names(correlations)=="gaba", , drop=FALSE]) # Still no significant (corr coef >0.5) GABA consumers
```

# Modeling GABA concentration with key species. 
