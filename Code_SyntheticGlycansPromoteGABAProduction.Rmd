...
biomass = read.csv("biomass-diff.csv", header = TRUE)
...
# Boxplot of each metabolite by treatment
for(mol in colnames(targeted_metabolomics)[2:10]){
    plot(ggboxplot(targeted_metabolomics_meta, x = "treatment", y = mol, xlab = "Treatment", ylab = "mg/mL", title = paste("Concentration of", mol, "by treatment")) + theme(axis.text.x = element_text(angle = 45, hjust = 1))
    )
}
```
...
# Remove columns 1-12 and acetic, butyric, l.glutamic, lactic, propionic, pyruvic, succinic, valeric
species_sgglc_subset = subset(species_sgglc, select = -c(1:12, acetic, butyric, l.glutamic, lactic, propionic, pyruvic, succinic, valeric))
